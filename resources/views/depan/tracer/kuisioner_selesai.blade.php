...
@section('konten')
    @role('mahasiswa')
        <div class="container">
            <div class="jumbotron">
                ...
                <p>Alumni <strong>{{ Auth::user()->name }}</strong>, Anda Telah menyelesaikan Proses Pengisian Kuisioner pada
                    {{ Carbon\Carbon::parse(Auth::user()->mahasiswa->updated_at)->format('d - M - Y') }}.</p>
            ...
            @if (Auth::user()->mahasiswa->f8->f8 == 1)
                <div class="jumbotron">
                    <h1>Lanjutkan untuk mengisi informasi Stakeholder Oleh pimpinan Anda</h1>
    ...
    @role('stakeholder')
        <div class="container">
            <div class="jumbotron">
                <h1>Terima Kasih Telah Mengisi Kuisioner</h1>
                <p>Yang Terhormat, <strong>{{ Auth::user()->name }}</strong>, Anda Telah menyelesaikan Proses Pengisian
                    Kuisioner.</p>
                <p>Cetak Bukti <a href="{{ route('cetak.stakeholder') }}"" class="  btn btn-info" target="new">DOWNLOAD</a>
                </p>
            </div>
        </div><!-- end content-->
    @endrole
