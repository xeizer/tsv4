...
<li class="treeview @isset($active) @if ($active == 4)active @endif @endisset">
    ...
    <ul class="treeview-menu">
        <li class="@isset($subactive) @if ($subactive == ' 4')active @endif @endisset">
            <a
                href="{{ route('statistik.tracer', ['prodi' => Auth::user()->admin->prodim_id, 'tahunangkatan' => 0, 'tahunlulus' => 0]) }}"><i
...
<li class="treeview @isset($active) @if ($active == 5)active @endif @endisset">
    ...
    <ul class="treeview-menu">
        @foreach (App\Prodim::where('id', Auth::user()->admin->prodi->id)->get() as $p)
            <li class="@isset($subactive) @if ($subactive == " 5$p->id")active @endif @endisset">
                <a
                    href="{{ route('stakeholder.index', ['prodi_id' => Auth::user()->admin->prodi->id, 'tahunangkatan' => 0, 'tahunlulus' => 0]) }}"><i
...
<li class="treeview @isset($active) @if ($active == 5)active @endif @endisset">
    ...
    <ul class="treeview-menu">
        @foreach (App\Prodim::where('id', Auth::user()->admin->prodi->id)->get() as $p)
            <li class="@isset($subactive) @if ($subactive == " 5$p->id")active @endif @endisset">
                <a
                    href="{{ route('pekerjaan.index', ['prodi_id' => $p->id, 'tahunangkatan' => 0, 'tahunlulus' => 0]) }}"><i
                        class="fa @isset($subactive) @if ($subactive == " 5$p->id")fa-circle @else fa-circle-o @endif
                    @endisset"></i>
                    {{ $p->slug_prodi }}
                </a>
